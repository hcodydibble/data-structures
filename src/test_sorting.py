...
"""The following are test functions for the radix sort algorithm."""
...
@pytest.mark.parametrize('the_list, result', RADIX_SORT_LIST)
def test_radix_sort_on_various_lists(the_list, result):
    """Same as the others honestly."""
    from radix_sort import radix_sort
    assert radix_sort(the_list) == result


def test_radix_sort_raises_error():
    """Test that merge sort will raise an error."""
    from radix_sort import radix_sort
    with pytest.raises(ValueError):
        radix_sort('BROKE AF')
